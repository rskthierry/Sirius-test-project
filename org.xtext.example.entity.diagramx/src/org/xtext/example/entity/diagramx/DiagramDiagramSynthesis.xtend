...
class DiagramDiagramSynthesis extends AbstractDiagramSynthesis<Diagram> {
	...
	def void createNamespaceFigures(EList<DNamespace> namespaces, KNode parentNode) {
		namespaces.forEach [ DNamespace dNamespace |
			val namespaceNode = dNamespace.createNode()
			namespaceNode.setLayoutOption(LayoutOptions.EDGE_ROUTING, EdgeRouting.ORTHOGONAL);
